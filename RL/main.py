...
dataset = GNNBenchmarkDataset(root='data/CLUSTER', name='CLUSTER', transform=OneHotDegree(190))
...
def prepare_testing_plot_data(test_dataset, limit, nnet):
    ...
    # save plot data
    # np.save('test_plots_data/test_dataset_graph_size.npy', test_dataset_graph_size)
    # np.save('test_plots_data/test_dataset_graph_density.npy', test_dataset_graph_density)
    # np.save('test_plots_data/smart_decomposition_cnt_lst.npy', smart_decomposition_cnt_lst)
    np.save('test_plots_data/smart_leaf_subgraph_lst.npy', smart_leaf_subgraph_lst)
    # np.save('test_plots_data/min_huristic_decomposition_cnt_lst.npy', min_huristic_decomposition_cnt_lst)
    np.save('test_plots_data/min_huristic_leaf_subgraph_lst.npy', min_huristic_leaf_subgraph_lst)
    # np.save('test_plots_data/max_huristic_decomposition_cnt_lst.npy', max_huristic_decomposition_cnt_lst)
    np.save('test_plots_data/max_huristic_leaf_subgraph_lst.npy', max_huristic_leaf_subgraph_lst)
    # np.save('test_plots_data/rand_huristic_decomposition_cnt_lst.npy', rand_huristic_decomposition_cnt_lst)
    np.save('test_plots_data/rand_huristic_leaf_subgraph_lst.npy', rand_huristic_leaf_subgraph_lst)
    # np.save('test_plots_data/smart_times.npy', smart_times)
    # np.save('test_plots_data/min_huristic_times.npy', min_huristic_times)
    # np.save('test_plots_data/max_huristic_times.npy', max_huristic_times)
    # np.save('test_plots_data/rand_huristic_times.npy', rand_huristic_times)
...
if __name__ == '__main__':
    ...
    nnet = NeuralNet(dataset.num_features)

    limit = 40  # used in solver
    rejection_lb = 80 # reject graphs with less than 80 nodes
    train_ratio = 0.8
    val_ratio = 0.1
    test_ratio = 0.1
    indices = []
    ...
    for i, data in enumerate(dataset):
        if data.num_nodes >= rejection_lb:
            indices.append(i)
    print(f'New dataset size: {len(indices)}')
    ...
    # Uncomment to test
    nnet = NeuralNet(dataset.num_features)
    nnet.load_checkpoint(filename='rejected_model5199_6.865145228215767.pth.tar')
    prepare_testing_plot_data(test_dataset, limit, nnet)
    print("Done")
    exit(0)


    timeout = 10  # seconds # Todo: adaptive timeout based on nodes and limit
