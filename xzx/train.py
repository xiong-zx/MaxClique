...
class SeparatorTrainEnv(gym.Env):
    """
    Description:
    ...
    #         decomposed_graphs = decompose_graph(
    #             self.current_graph, self.separator)
    #         reward += -np.sum(np.exp([len(g)/len(self.current_graph) for g in decomposed_graphs]))
    #         return reward, {
    #             'epoch': self.epoch,
    #             'nodes_picked': np.sum((action == 1)),
    #             'invalid_nodes_picked': invalid_action_count,
...
def plot_train(info: dict, figure_dir: str):
    ...
    avg_node_reduced = pd.DataFrame(avg_node_reduced)
    avg_separator_size = np.mean(separator_size[:, :total_epochs-1], axis=0)
    avg_separator_size = pd.DataFrame(avg_separator_size)
    
    avg_node_reduced.to_csv(os.path.join(figure_dir, 'train_node_reduced.csv'))
    avg_separator_size.to_csv(os.path.join(figure_dir, 'train_separator_size.csv'))
    ...
    plt.title(f'Histogram of Node Reduced (epoch {best_epoch}')
    ...
    plt.ylabel('Number of Graphs')
    plt.title(f'Histogram of Separator Size (epoch {best_epoch}')
...
if __name__ == "__main__":
    ...
    # hyperparameters
    total_steps = int(1e6)  # total number of training steps
    interval = int(1e3)  # number of steps between each test
    val_steps = int(1e2)  # number of steps for each test; None for validating for 1 epoch
    ...
    # data_base_dir = os.path.join(_root, 'data', 'CLUSTER')
    ...
    data_base_dir = os.path.join(_root, 'data', '100', '0.1')
    train_data_list = [os.path.join(data_base_dir,'train', f) for f in os.listdir(
        data_base_dir + '/train') if f.endswith('.edgelist')]
    val_data_list = [os.path.join(data_base_dir,'val', f) for f in os.listdir(
        data_base_dir + '/val') if f.endswith('.edgelist')]
    test_data_list = [os.path.join(data_base_dir,'test', f) for f in os.listdir(
        data_base_dir + '/test') if f.endswith('.edgelist')]
    
    train_dataset = Dataset(train_data_list, root=None)
    val_dataset = Dataset(val_data_list, root=None)
    test_dataset = Dataset(test_data_list, root=None)
    
    # By now, the train_dataset and val_dataset should be ready.

    train_env = make_vec_env(lambda: SeparatorTrainEnv(
        dataset=train_dataset, node_embedding=False), n_envs=1)
    test_env = make_vec_env(lambda: SeparatorTrainEnv(
        dataset=test_dataset, node_embedding=False), n_envs=1)
    if os.path.exists(os.path.join(data_base_dir, "a2c_vertex_separator.zip")):
        msg = input("Model exists. Press Y/y to load the model.")
        if msg.lower() == 'y':
            print("Loading model.")
            model = A2C.load(path=os.path.join(
                data_base_dir, "a2c_vertex_separator"), verbose=0)
            model.env = train_env
            print("Model loaded.")
        else:
    ...
    for i in range(0, total_steps, interval):
        ...
        print(f"Testing.")
        val_env = make_vec_env(lambda: SeparatorTrainEnv(
        ...
        if node_reduced >= best_node_reduced:
            ...
            model.save(path=os.path.join(data_base_dir, "a2c_vertex_separator"))
            print("Model saved.")
    ...
    plot_train(info=train_info, figure_dir=data_base_dir)
    ...
    plt.savefig(os.path.join(data_base_dir, 'val_line_node_reduced.png'))
    plt.close()
    ...
    plt.savefig(os.path.join(data_base_dir, 'val_line_separator_size.png'))
    plt.close()
    ...
    val_node_reduced_list.to_csv(os.path.join(data_base_dir, 'val_node_reduced.csv'))
    val_separator_size_list.to_csv(os.path.join(data_base_dir, 'val_separator_size.csv'))
    test_node_reduced.to_csv(os.path.join(data_base_dir, 'test_node_reduced.csv'))
    test_separator_size.to_csv(os.path.join(data_base_dir, 'test_separator_size.csv'))
